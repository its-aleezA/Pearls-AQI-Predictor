...
try:
    ...
    if "No hudi properties" in str(e) or "no data has been written" in str(e).lower():
        ...
        print(f"✅ Loaded {len(df)} rows from aqi_history.csv")
    else:
        raise
...
# Only drop rows where core features are missing (e.g. aqi, temp, humidity); going to impute the rest (e.g. pm10/no2/o3 missing from WAQI fallback) with 0 or median values later to preserve as much data as possible for training
...
try:
    ...
    WINDOW = 24  # use past 24 hours to predict next hours AQI
...
# 9. Register in Hopsworks Model Registry (with metrics and SHAP plot as artifact)
